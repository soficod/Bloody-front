...
const Chirurgie = ({parents,maladies,setMaladies})=>{
    ...
    useEffect(()=>{
        if(typeof(parents) != "undefined")
        {
            setLocalMaladie(parents.filter(m=>{
                return m.id == 4 || m.id == 5 || m.id == 6 || m.id == 7 || m.id == 8 || m.id == 9 || m.id == 10 || m.id == 11 || m.id == 4 || m.id == 12 || m.id == 13; 
            }))
        }
    },[parents])
...
return(
    <Box sx={{
        ...
        '&> :not(style)' :{width:'100%'}
            ...
            <h1 style={{textAlign:"center",marginBottom:"60px",height:"50px",color:"rgb(79, 78, 78,0.9", backgroundColor:"rgb(143, 199, 255,0.2)"}}>Chirurgie</h1>
        <Box sx={{
            my:3
        }}>
                      <Box sx={{
